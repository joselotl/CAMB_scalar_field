    ...
    logical :: quintessence = .false.  !If true Om_Lambda is interpreted as quintessence, otherwise just Lambda
    ...
    use LambdaGeneral
    implicit none
    integer, parameter :: NumPoints = 2000, NumPointsEx=NumPoints+2
    !We actually calulate NumPointEx-NumPoints points after a=1 to get good interpolation at a=1
    real(dl) theta_a(NumPointsEx), y1_a(NumPointsEx), Omegaphi_a(NumPointsEx),NVals(NumPointsEx)
    real(dl) dtheta_a(NumPointsEx), dy1_a(NumPointsEx), dOmegaphi_a(NumPointsEx), auxiliar_spline(NumPointsEx)
    real(dl), parameter :: theta_aux2 = 1d4
    !real(dl), parameter :: theta_aux2 = 1d4

    real(dl) adot, factor_medio

    real(dl) :: dN, masa, Nstart
    real(dl) :: sigma_model =  1.2247

    contains


    function sinstar(angulo)
       real(dl) sinstar, angulo
       real(dl) smoothfactor
       smoothfactor = 0.5d0*(1.0d0-tanh(angulo**2 - theta_aux2))
       sinstar = smoothfactor*sin(angulo)
    end function sinstar

    function cosstar(angulo)
       real(dl) cosstar, angulo
       real(dl) smoothfactor
       smoothfactor = 0.5d0*(1.0d0-tanh(angulo**2 - theta_aux2))
       cosstar = smoothfactor*cos(angulo)
    end function cosstar



    !Jos: Aqui colocaremos las ecuaciones de evolucion
    subroutine EvolveBackground(dum,num,N,y,yprime)
    use MassiveNu
    implicit none
    real dum
    integer num
    ...
    real(dl) theta_scalar,y1_scalar, Omega_scalar
    real(dl) w_scalar, w_tot, dens_extra, p_extra !, smoothfactor
    integer nu_i
    ...
    y1_scalar = y(2)
    Omega_scalar = y(3)
    !Jos: Ahora necesito calcular la ecuacion de estado
    ! supondre que estamos en el caso plano, esto no puede mejorarse porque
    ! creo yo que es necesario para que salga el sistema autonomo
    dens_extra = grhob*a + grhog + grhornomass + grhok*a2 + grhov*a2**2
    p_extra = (grhog+grhornomass)/3 - grhov*a2**2
    if (CP%Num_Nu_massive /= 0) then
        !Get massive neutrino density relative to massless
        do nu_i = 1, CP%nu_mass_eigenstates
            call Nu_background(a*nu_masses(nu_i),rhonu,pnu)
            dens_extra = dens_extra + rhonu*grhormass(nu_i)
            p_extra = p_extra + pnu*grhormass(nu_i)
        end do
    end if
    !Jos: Estoy suavizando las funciones trigonométricas, el smoothfactor es artificial
    !smoothfactor = 0.5d0*(1.0d0-tanh(theta_scalar**2 - theta_aux2))
    w_scalar = -cosstar(theta_scalar) 
    w_tot = (1.0d0 - Omega_scalar)*p_extra/dens_extra + Omega_scalar*w_scalar
    
    yprime(1) = -3.0d0*sinstar(theta_scalar) + y1_scalar
    yprime(2) = 1.5d0*(1.0d0 + w_tot)*y1_scalar
    yprime(3) = 3.0d0*(w_tot-w_scalar)*Omega_scalar

    end subroutine EvolveBackground


    !Jos: Sera buena idea usar N en vez de a?
    !function Quint_GetOmegaFromInitial(astart,phi,phidot,atol)
    function Quint_GetOmegaFromInitial(Nstart,theta_quint,y1_quint,Omega_quint,atol)
    !Get CP%omegav today given particular conditions phi and phidot at a=astart
    !real(dl), intent(IN) :: Nstart, phi,phidot, atol
    real(dl), intent(IN) :: Nstart, theta_quint, y1_quint, Omega_quint, atol
    real(dl) Quint_GetOmegaFromInitial
    !Jos: Aqui cambio el numero de ecuaciones
    integer, parameter ::  NumEqs=3
    real(dl) c(24),w(NumEqs,9), y(NumEqs), nst, nend
    integer ind, i
    real dum
    ...
    y(2) = y1_quint
    y(3) = Omega_quint
    !call dverk(dum,NumEqs,EvolveBackground,nst,y,0._dl,atol,ind,c,NumEqs,w)
    !call EvolveBackground(dum,NumEqs,0._dl,y,w(:,1))
    dN = -Nstart/2000.
    do i=1, 2000
       nst = Nstart + dN*(i-1)
       nend = Nstart+dN*i
       call dverk(dum,NumEqs,EvolveBackground,nst,y,nend,atol,ind,c,NumEqs,w)
       call EvolveBackground(dum,NumEqs,nend,y,w(:,1))
      ! print*, nst,nend
    end do

    Quint_GetOmegaFromInitial=y(3)
    ...
    real(dl) theta_quint, y1_quint, Omega_quint

    real(dl) rhonu, dens_rel, hubbleini
    real(dl) y1_ini, theta_ini, Omega_ini, factor_bajo, factor_alto
    !real(dl) densitemp
    integer nu_i
    !Jos output file
    integer, parameter :: out_unit=18
    open (unit=out_unit,file="background_phi.dat",action="write",status="replace")
    ...
    astart=1d-9
    atol=1d-8
    !Jos: Aqui defino la masa del campo escalar
    masa = 1.56375d3 !10**-26 eV en unidades de Mpc**-1
    !masa = masa*1.0d1 !10**-25 eV en unidades de Mpc**-1
    !masa = masa*1.0d3 !10**-23 eV en unidades de Mpc**-1
    masa = masa*1.0d2 !10**-24 eV en unidades de Mpc**-1
    !masa = masa*1.0d6 !10**-20 eV en unidades de Mpc**-1
    !masa = masa*1.0d4 !10**-22 eV en unidades de Mpc**-1
    !masa = masa*1.0d0 !Buscando que la oscilacion inicie igual que con Alma
    print*, 'lamasamamamaa', masa

    !Jos: Aqui pongo las condiciones iniciales obtenidas por Alma y Luis
    ! requiero obtener el factor de Hubble inicial, para ello calculo la densidad
    ! no necesito incluir la materia ni la constante cosmologina en estos tiempos
    Nstart=log(astart)
    dens_rel = grhog+grhornomass
    if (CP%Num_Nu_massive /= 0) then
        !Get massive neutrino density relative to massless
        do nu_i = 1, CP%nu_mass_eigenstates
            call Nu_rho(astart*nu_masses(nu_i),rhonu)
            dens_rel = dens_rel + rhonu*grhormass(nu_i)
        end do
    end if
    hubbleini = sqrt(dens_rel/3.0d0)/(astart**2)
    y1_ini = 2.0d0 * masa/hubbleini
    if (y1_ini > 0.02) then
       write(*,*) 'The initial background time is too late.'
       write(*,*) 'y1_ini, a_ini= ', real(y1_ini), real(astart)
    end if
    theta_ini = y1_ini/5.0d0
    Omega_ini =  astart * grhoc / dens_rel * (2.0d0*theta_ini/pi)**1.5d0 * &
       ((9.0d0 + 0.25d0*pi**2)/(9.0d0 + theta_ini**2))**0.75d0
    print*, 'Esto es omegaini = ', Omega_ini
    print*, 'Esto es Nstart = ', Nstart
    print*, 'Esto es la omega que me piden = ',CP%omegac


    !Jos: A continuacion ejecuto un metodo de shooting para encontrar unas condiciones
    ! iniciales mas precisas
    factor_bajo=1.0d-1
    factor_alto=1.0d2
    om1= Quint_GetOmegaFromInitial(Nstart,theta_ini,y1_ini,Omega_ini*factor_bajo, atol)
   if (abs(om1-CP%omegac) > 1d-3) then
      !if not, do binary search in the interval
      OK=.false.
      om2= Quint_GetOmegaFromInitial(Nstart,theta_ini,y1_ini,Omega_ini*factor_alto, atol)
      if (om1 > CP%omegac .or. om2 < CP%omegac) then
          write (*,*) 'Initial values must bracket required value.  '
          write (*,*) 'om1, om2 = ', real(om1), real(om2)
          stop
      end if
      do iter=1,100
          factor_medio = (factor_alto + factor_bajo)/2
          om = Quint_GetOmegaFromInitial(Nstart,theta_ini,y1_ini,Omega_ini*factor_medio, atol)
          if (om < CP%omegac) then
              om1=om
              factor_bajo = factor_medio
          else
              om2=om
              factor_alto = factor_medio
          end if
          if (om2-om1 < 1d-4) then
             print*, Omega_ini*factor_medio
              OK=.true.
              exit
          end if
   ...
   print*, 'Omega_phi(z=0) = ', om
   Omega_ini = Omega_ini * factor_medio
   if (FeedbackLevel > 0) write(*,*) 'Omega_ini = ', Omega_ini
    ...
    y1_a(1) = y(2)
    Omegaphi_a(1) = y(3)
    dN = - Nstart/(dble(NumPoints) - 1.0d0)  !algo_a(NumPoints+1) is value now
    NVals(1)=Nstart
    write(out_unit, *) NVals(1), Omegaphi_a(1), y1_a(1), theta_a(1)
    !Jos: El siguiente do debe generar dos puntos con N mayor que 0
    do i=2, NumPointsEx
        Nend = dN*dble(i-NumPoints)
        NVals(i)=Nend
        call dverk(dum,NumEqs,EvolveBackground,Nstart,y,Nend,atol,ind,c,NumEqs,w)
        !Jos: Lo que hace a continuacion es actualizar el espacio de trabajo, lo cual permite que
        ! se mantenga la precision del metodo Runge Kutta...
        call EvolveBackground(dum,NumEqs,Nend,y,w(:,1))
        theta_a(i) = y(1)
        y1_a(i) = y(2)
        Omegaphi_a(i) = y(3)
        write(out_unit, *) NVals(i)*0.4343, Omegaphi_a(i), y1_a(i), theta_a(i), (2.0d0*masa/y1_a(i))**2*Omegaphi_a(i)
        !Jos: Escribire aqui la presion y densidad
        !densitemp=12.0d0*Omega_phi*(masa*ande/y1_phi)**2
        !write(out_unit, *) densitemp,-densitemp*cosstar(theta_phi), y1_a(i), theta_a(i)
    end do

    call splini(auxiliar_spline,NumPointsEx)
    call splder(theta_a,dtheta_a,NumPointsEx,auxiliar_spline)
    call splder(y1_a,dy1_a,NumPointsEx,auxiliar_spline)
    call splder(Omegaphi_a,dOmegaphi_a,NumPointsEx,auxiliar_spline)
    !splZero=0
    !call spline(NVals,theta_a,NumPointsEx,splZero,splZero,ddtheta_a)
    !call spline(NVals,y1_a,NumPointsEx,splZero,splZero,ddy1_a)
    !call spline(NVals,Omegaphi_a,NumPointsEx,splZero,splZero,ddOmegaphi_a)

    end subroutine Quint_init_background

    subroutine Quint_ValsAta(NN,Omegaphi_N,y1_N,theta_N)
       use MassiveNu
    implicit none
    !Jos: No estoy seguro que el algoritmo original sea correcto, mejor uso el que viene en
    ! la pagina de mathematica, y que tambien se usa en Nu_background
    real(dl) NN, Omegaphi_N, y1_N, theta_N
    real(dl) tt, dens_rel, astar, hubbleini, rhonu
    integer ix, nu_i
    ...
    if (NN>Nstart) then
    ix = int(NN/dN + NumPoints) 
 !   print*, 'Llamada a interpolacion ix, tt, NN', ix, tt, NN
    tt = (NN - NVals(ix))/dN
    if (tt < -2.0d-14 .or. tt > 1.0d0) then 
       print*, 'Se pidio una N,ix,NVails,tt',NN,ix,NVals(ix),tt
       print*, 'Se pidio una N,ix,NVails,tt',dN,NN/dN+NumPoints
       stop 'Error en la interpolacion'
    end if
    Omegaphi_N = Omegaphi_a(ix) + tt * (dOmegaphi_a(ix) + &
       tt * (3.0d0*(Omegaphi_a(ix+1) - Omegaphi_a(ix)) - 2.0d0*dOmegaphi_a(ix) - dOmegaphi_a(ix+1) + &
       tt * (2.0d0*(-Omegaphi_a(ix+1) + Omegaphi_a(ix)) + dOmegaphi_a(ix) + dOmegaphi_a(ix+1) )))
    y1_N = y1_a(ix) + tt * (dy1_a(ix) + &
       tt * (3.0d0*(y1_a(ix+1) - y1_a(ix)) - 2.0d0*dy1_a(ix) - dy1_a(ix+1) + &
       tt * (2.0d0*(-y1_a(ix+1) + y1_a(ix)) + dy1_a(ix) + dy1_a(ix+1) )))
    theta_N = theta_a(ix) + tt * (dtheta_a(ix) + &
       tt * (3.0d0*(theta_a(ix+1) - theta_a(ix)) - 2.0d0*dtheta_a(ix) - dtheta_a(ix+1) + &
       tt * (2.0d0*(-theta_a(ix+1) + theta_a(ix)) + dtheta_a(ix) + dtheta_a(ix+1) )))

 else
    !Jos: Si se pide N mas chica
    astar = exp(NN)
    dens_rel = grhog+grhornomass
    if (CP%Num_Nu_massive /= 0) then
        !Get massive neutrino density relative to massless
        do nu_i = 1, CP%nu_mass_eigenstates
            call Nu_rho(astar*nu_masses(nu_i),rhonu)
            dens_rel = dens_rel + rhonu*grhormass(nu_i)
        end do
    end if
    hubbleini = sqrt(dens_rel/3.0d0)/(astar**2)
    y1_N = 2.0d0 * masa/hubbleini
    theta_N = y1_N/5.0d0
    !Jos: Aqui necesito alguna forma de extender la omega inicial, la cual obtuvimos por el metodo de shooting
    Omegaphi_N =  astar * grhoc / dens_rel * (2.0d0*theta_N/pi)**1.5d0 * &
       ((9.0d0 + 0.25d0*pi**2)/(9.0d0 + theta_N**2))**0.75d0
    Omegaphi_N = Omegaphi_N*factor_medio
    ...
    integer nu_i
    real(dl) Omega_phi, y1_phi, theta_phi
    real(dl) NNN
    ...
    !  8*pi*G*rho*a**4.
    grhoa2 = grhok*a2 + grhob*a + grhog + grhornomass + grhov*a2**2
    if (CP%Num_Nu_massive /= 0) then
        !Get massive neutrino density relative to massless
        do nu_i = 1, CP%nu_mass_eigenstates
  ...
  if (a.ne.0.0d0) then
    call Quint_ValsAta(NNN,Omega_phi,y1_phi,theta_phi)
    dtauda=sqrt(3.0d0*(1.0d0-Omega_phi)/grhoa2)
 else
    ...
    real(dl) ISW
    ! Jos: Voy a usar deltaphi y yphi como variables
    !real(dl) nu_phi, alpha_phi
    real(dl) deltaphi, xphi
    real(dl) phidot, grhoex_t
    real(dl) NNN, Omega_phi, y1_phi, theta_phi
    real(dl) dtauda
    ...
    grhob_t=grhob/a
    call Quint_ValsAta(NNN,Omega_phi,y1_phi,theta_phi)
    grhoc_t = 3.0d0*Omega_phi/dtauda(a)**2
    grhor_t=grhornomass/a2
    grhog_t=grhog/a2
    grhov_t=grhov*a2
    grho=grhob_t+grhoc_t+grhor_t+grhog_t+grhov_t
    gpres=(grhog_t+grhor_t)/3-grhov_t-grhoc_t*cosstar(theta_phi)

    !  8*pi*a*a*SUM[rho_i*clx_i] add radiation later
    dgrho=grhob_t*clxb+grhoc_t*clxc

    !  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
    !Jos: Cambiando la expresion usando algunas identidades trigonometricas
    !dgq=grhob_t*vb - grhoc_t*k2/(a*masa)*sinstar(0.5*theta_phi)*(sinstar(0.5*theta_phi)*xphi+cosstar(0.5*theta_phi)*deltaphi)
    dgq=grhob_t*vb 
    ...
    dgq = dgq - grhoc_t*k2*dtauda(a)/y1_phi*(xphi*(1.0d0-cosstar(theta_phi)) + deltaphi*sinstar(theta_phi))
    ...
    integer, parameter :: i_clxg=1,i_clxr=2,i_clxc=3, i_clxb=4, &
        i_qg=5,i_qr=6,i_vb=7,i_pir=8, i_eta=9, i_aj3r=10,i_xphi=11
    integer, parameter :: i_max = 12
    ...
    !print*, 'Esta es la a desde donde se empiezan las perturbaciones',a
    NNN=log(a)
    call Quint_ValsAta(NNN, Omega_phi,y1_phi,theta_phi)
    if (y1_phi > 0.02) then
       write(*,*) 'The initial perturbative time is too late.'
       write(*,*) 'y1_ini, a_ini, k = ', real(y1_phi), real(a), real(k)
    end if
    initv(1,i_clxg)=-chi*EV%Kf(1)/3*x2*(1-omtau/5)
    initv(1,i_clxr)= initv(1,i_clxg)
    initv(1,i_clxb)=0.75_dl*initv(1,i_clxg)
    ! initv(1,i_clxc)=3.0d0*theta_phi*initv(1,i_clxg)/14.0d0
    initv(1,i_clxc)=theta_phi**2*initv(1,i_clxg)/56.0d0
    ...
    initv(1,i_xphi) = - 3.0d0*theta_phi*initv(1,i_clxg)/14.0d0
    ...
    !  CDM
    !Jos: No quitare esto, espero que no pase nada
    y(3)=InitVec(i_clxc)
    ...
    y(EV%w_ix) = InitVec(i_xphi)
    y(EV%w_ix+1) = -3.0d0/2.0d0*InitVec(i_clxg)
    !  Neutrinos
    ...
    real(dl) NNN, Omega_phi, y1_phi, theta_phi
    real(dl) deltaphi, xphi
    real(dl) dobleu_porache, hprimaema
    real(dl) uno,dos,tres, cuatro
    real(dl) dtauda
    ...
    NNN = log(a)
    call Quint_ValsAta(NNN,Omega_phi,y1_phi,theta_phi)
    !grhoc_t = 12.0d0*Omega_phi*(masa*a/y1_phi)**2
    grhoc_t = 3.0d0*Omega_phi/dtauda(a)**2
    grhor_t=grhornomass/a2
    grhog_t=grhog/a2
    grhov_t=grhov*a2
    ...
    !  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
    !dgq=grhob_t*vb-grhoc_t*k2/(a*masa)*sinstar(0.5d0*theta_phi)*(sinstar(0.5d0*theta_phi)*xphi + cosstar(0.5d0*theta_phi)*deltaphi)
    dgq=grhob_t*vb 
    ...
    !dgq = dgq - grhoc_t*k2/(2.0d0*a*masa)*(xphi*(1.0d0-cosstar(theta_phi)) + deltaphi*sinstar(theta_phi))
    dgq = dgq - grhoc_t*k2*dtauda(a)/y1_phi*(xphi*(1.0d0-cosstar(theta_phi)) + deltaphi*sinstar(theta_phi))
    ...
    !Jos: Aqui pongo el factor 0.5*dh/dtau y tambien w*H/2
    hprimaema = k*z
    dobleu_porache = 0.5d0*k2/(masa*a)
    dobleu_porache = k2*dtauda(a)/y1_phi
    !  CDM equation of motion
    ! clxcdot = - dobleu_porache*(1.0d0 - cosstar(theta_phi)) * xphi &
    !     + dobleu_porache*sinstar(theta_phi)*deltaphi 
    !     !- hprimaema * (1.0d0 - cosstar(theta_phi))
    ! ayprime(3)= clxcdot
    ! ayprime(EV%w_ix)= - dobleu_porache*sinstar(theta_phi) * xphi &
    !    - dobleu_porache*(1.0d0 + cosstar(theta_phi))*deltaphi 
    !    ! + hprimaema*sinstar(theta_phi)
    clxcdot = (3.0d0*sinstar(theta_phi)*adotoa + dobleu_porache*(1.0d0 - cosstar(theta_phi))) * xphi &
        + dobleu_porache*sinstar(theta_phi)*deltaphi &
        - hprimaema * (1.0d0 - cosstar(theta_phi))
    ayprime(3)= clxcdot
    ayprime(EV%w_ix)= -(3.0d0*cosstar(theta_phi)*adotoa + dobleu_porache*sinstar(theta_phi)) * xphi &
        - dobleu_porache*(1.0d0 + cosstar(theta_phi))*deltaphi &
        + hprimaema*sinstar(theta_phi)
    ayprime(EV%w_ix+1) = 2.0d0*hprimaema
    ...
    if (EV%TightCoupling) then
        !  ddota/a
        gpres=gpres+ (grhog_t+grhor_t)/3 - grhoc_t*cosstar(theta_phi) - grhov_t
