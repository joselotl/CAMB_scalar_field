    ...
    !real(dl) :: fixq = 10.0d0 !Debug output of one q
    !real(dl) :: fixq = 10.0d0 !Debug output of one q
    !real(dl) :: fixq = 0.0d0 !Debug output of one q
    real(dl) :: fixq = 5.0d0 !Debug output of one q
    ...
    if (CP%WantCls) then
        ...
        do q_ix= 1,Evolve_q%npoints
            if (global_error_flag==0) then
               ...
               !print*, 'Estoy calculando aqui', q_ix
            end if
        end do
        !$OMP END PARAllEl DO
    ...
    real(dl) Naux, Omegaaux,y1aux,thetaaux, nuaux, alphaaux
    real(dl) Nfin
    real(dl) aux1, aux2, aux3, aux4, aux5, aux6, dobleuporache, hprima, ache, dgrho, etakk,a,zetak
    external dtauda
  !  print*, 'Ahora estoy evolucionando', EV%q
    ...
    !Jos: Voy a usar esto. Ya me vi
    ! Ahora voy a cambiar los momentos en que se evaluan para tener mas evaluaciones al inicio
    !print*,'Estoy aqui acaso', EV%q
    !fixq=1.0
    if (fixq/=0._dl) then
        tol1=tol/exp(AccuracyBoost-1)
        ! call CreateTxtFile('tests/evolveN.txt',1)
        ! call CreateTxtFile('tests/evolvedelta.txt',23)
        ! call CreateTxtFile('tests/terminos.txt',45)
        open (unit=23, file="pruebas/perturbaciones_k.dat", action="write", status="replace")
        open (unit=25, file="pruebas/background.dat", action="write", status="replace")
            call GaugeInterface_EvolveScal(EV,tau,y,tau,tol1,ind,c,w)
            yprime = 0
            ...
            !Jos: Aqui quiero tambien calcular la parte del background
            Naux=log(y(1))
            a = y(1)
            nuaux = y(EV%w_ix)
            
            call Quint_ValsAta( Naux, Omegaaux,y1aux,thetaaux)
            !Jos: A continuacion escribo los terminos de las ecuaciones diferenciales
            ache = 2*y(1)*masa/y1aux
            etakk = y(2)


            ! dgrho=grhob/a*y(4) &
            !    - exp(alphaaux)*sin(0.5d0*nuaux)* 12.0d0*Omegaaux*(masa*a/y1aux)**2
            ! zetak= - yprime(3)
            ! dobleuporache = fixq**2/(masa*y(1))/ache
            ! hprima = 2.0d0*zetak*exp(-alphaaux)/ache

            ! aux1 = -3.0d0*(sinstar(thetaaux)+sinstar(thetaaux-nuaux))
            ! aux2 = -(1.0d0 - cosstar(thetaaux - nuaux))*dobleuporache
            ! aux3 =  hprima*(cos(0.5d0*nuaux)-cosstar(thetaaux-0.5d0*nuaux))
            ! aux4 = -1.5d0*(cosstar(thetaaux-nuaux) + cosstar(thetaaux))
            ! aux5 = -0.5d0*dobleuporache*sinstar(thetaaux-nuaux)
            ! aux6 = 0.5d0*hprima*(sin(0.5d0*nuaux)+sinstar(thetaaux-0.5d0*nuaux))


        do j=1,1000
        !print*,'Wau'
        !print*, y(1), log(y(1)), y(EV%w_ix), y(EV%w_ix+1), y(4)
        Nfin = -15.0d0*(1000-j)/1000
        tauend = TimeOfz(exp(-Nfin)-1.0d0)
            ...
            Naux=log(y(1))
            call Quint_ValsAta( Naux, Omegaaux,y1aux,thetaaux)
            write (23,'(7E15.5)') Naux*0.4343, y(3), y(4),y(EV%w_ix+1), y(EV%w_ix),cosstar(thetaaux)
            write (25,'(7E15.5)') Naux*0.4343, y(1),Omegaaux,thetaaux, y1aux*(2.0d0*y(1)**2*dtauda(y(1)))
            !write (23,'(7E15.5)') Naux*0.4343, y(3), y(4),y(2), y(EV%w_ix),thetaaux
            !write (23,'(7E15.5)') Naux*0.4343, y(3), y(4),y(2), y(EV%w_ix),thetaaux
            !write (23,'(7E15.5)') Naux*0.4343,y(4), - exp(y(EV%w_ix+1))*sin(0.5d0*y(EV%w_ix)), y(EV%g_ix)
            ! nuaux = y(EV%w_ix)
            ! alphaaux = y(EV%w_ix+1)
            ! ache = 2*y(1)*masa/y1aux
            ! etakk = y(2)
            ...
            !    - exp(alphaaux)*sin(0.5d0*nuaux)* 12.0d0*Omegaaux*(masa*a/y1aux)**2
            ! zetak= - yprime(3)
            ! dobleuporache = fixq**2/(masa*y(1))/adotoa
            ! hprima = 2.0d0*zetak*exp(-alphaaux)/ache
            ...
            ! aux5 = -0.5d0*dobleuporache*sinstar(thetaaux-nuaux)
            ! aux6 = 0.5d0*hprima*(sin(0.5d0*nuaux)+sinstar(thetaaux-0.5d0*nuaux))
            ! write (45,'(7E15.5)') Naux*0.4343,aux1, aux2, aux3, aux4, aux5, aux6
        end do
        close(23)
        stop
    end if
    ...
    !Jos: No se para que se necesite esto o si lo habre borrado antes

            call GaugeInterface_EvolveScal(EV,tau,y,tau,tol1,ind,c,w)
            yprime = 0
            call derivs(EV,EV%ScalEqsToPropagate,tau,y,yprime)

    !print*, TimeSteps%npoints, tol1,tau,TimeSteps%points(1), TimeSteps%points(2)
    !print*, TimeOfz(1/y(1)-1.0d0)
    !print*, 'primer paso',log(y(1)), y(EV%w_ix), y(EV%w_ix+1) 
    ...
    !print*, 'segundo paso',log(y(1)), y(EV%w_ix), y(EV%w_ix+1),tau 
            call GaugeInterface_EvolveScal(EV,tau,y,tauend,tol1,ind,c,w)
            yprime = 0
            call derivs(EV,EV%ScalEqsToPropagate,tau,y,yprime)
    !print*, 'tercer paso',log(y(1)), y(EV%w_ix), y(EV%w_ix+1) , tau
    !stop
            if (global_error_flag/=0) return
            ...
            !Jos: aqui voy probando
 !           if (j==TimeSteps%npoints -3) then
 !           print*,y(1), y(EV%w_ix+1), y(3)
 !           end if
...
101         if (CP%WantTransfer.and.itf <= CP%Transfer%num_redshifts) then
    ...
    if (global_error_flag/=0) return
    call GaugeInterface_EvolveScal(EV,tau,y,tau,atol,ind,c,w)
