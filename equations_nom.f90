    ...
    !real(dl), parameter :: theta_aux2 = 1d8
 ...
 !   print*, 'Llamada a interpolacion ix, tt, NN', ix, tt, NN
  ...
  if (a.ne.0.0d0) then
    ...
    grhob_t=grhob/a
    call Quint_ValsAta(NNN,Omega_phi,y1_phi,theta_phi)
    grhoc_t = 3.0d0*Omega_phi*(a/dtauda(a))**2
    ...
    dgq=grhob_t*vb - grhoc_t*k2*dtauda(a)/y1_phi*(xphi*(1.0d0-cosstar(theta_phi)) + deltaphi*sinstar(theta_phi))
    ...
    integer, parameter :: i_max = i_xphi
    ...
    !dgq=grhob_t*vb - grhoc_t*k2/(2.0d0*a*masa)*(xphi*(1.0d0-cosstar(theta_phi)) + deltaphi*sinstar(theta_phi))
    dgq=grhob_t*vb - grhoc_t*k2*dtauda(a)/y1_phi*(xphi*(1.0d0-cosstar(theta_phi)) + deltaphi*sinstar(theta_phi))
    ...
    hprimaema = k*z
    dobleu_porache = 0.5d0*k2*dtauda(a)/(y1_phi*a)
    ...
    if (EV%TightCoupling) then
        !  ddota/a
        gpres=gpres+ (grhog_t+grhor_t)/3 - grhoc_t*cosstar(theta_phi)
